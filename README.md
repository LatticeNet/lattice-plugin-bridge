...
<template>
  <PcWorkspace>
    ...
    <PcProofLine :segments="['observed at 23:21:14', '25 nodes report', 'liveness: 138 running']" :refreshing="refreshing" />
...
- `PcWorkspace` is the page frame (`batch` keeps room for a batch bar). `PcPageHeader` takes `title`, `badge`, `description`, an `icon` component or `#icon` slot, and `#actions` for the page-level Refresh. `PcProofLine` prints `segments` joined by a middle dot, plus "refreshing".
...
- `PcModal` (`open`, `title`, `description`, `size` small, default, large, `returnFocusTo`, emits `close`; `#footer`), `PcSidePanel` (the same with `size` record or output), `PcConfirmDialog` (`open`, `title`, `message`, `confirmLabel`, `cancelLabel`, `destructive`, `busy`, emits `confirm` and `cancel`), `PcBatchBar` (`count`, emits `clear`).
